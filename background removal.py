...
while True:
    ...
    if status:
        ...
        frame = cv2.inRange(frame_rgb, lower_bound, upper_bound)
        # invertendo a máscara
        upper_bound = upper_bound + lower_bound
        # bitwise_and - operação para extrair o primeiro plano / pessoa
        res_1 = cv2.bitwise_and(frame, frame, lower_bound = upper_bound)
        # imagem final
        final_image = np.where(camera == 0, camera, lower_bound)
